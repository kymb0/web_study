...
// the security issue with this code lies on line 3, where we can see that a strong secret is only used if the environment is production, so if it were to be say, dev, test, staging etc - only the string 'secret' would be used for const secret. The second issue is that there are no complexity checks for the tsecret, indicating a weak secret may be used.
